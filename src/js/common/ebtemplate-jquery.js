const Ebtemplate = (function() {
    ...
    Ebtemplate.prototype = {
        ...
        getRequestURLofJSONP: function() {
            // 目标链接
            const targetURL = this.requestOptions.targetURL;
            // 请求参数
            const parameters = this.requestOptions.parameters;
            // 参数的键组
            const parametersKey = typeof parameters === 'object' ? Object.keys(parameters) : {};
            // 参数键值对字符串
            const parametersString = !(parametersKey instanceof Array) ? '' : parametersKey.reduce(function (string, key) {
        ...
        getTemplateString: function(templateID) {
            if(templateID !== undefined) {
                const templateBlock = $(document).find('script#' + templateID + '[data-eb-template]');
                const templateString = templateBlock.html();  // TODO: 去除标签间的空格
    ...
    Ebtemplate.prototype.constructor.processor = function processor(templateObjects) {
        for(let i = 0, templateObject; (templateObject = templateObjects[i++]);) {
            // TODO: 未判断
    ...
    Ebtemplate.prototype.constructor.requestData = function requestData(templateObject) {
        ...
        const $container = templateObject.$container;   // JQuery对象
        let paramPage = undefined;
        let paramSize = undefined;
        if(templateObject.requestOptions.parameters !== undefined) {
            paramPage = typeof templateObject.requestOptions.parameters.page === 'number' && templateObject.requestOptions.parameters.page;
            paramSize = typeof templateObject.requestOptions.parameters.size === 'number' && templateObject.requestOptions.parameters.size;
        }

        const templateString = templateObject.templateString;
        const method = templateObject.requestOptions.method;
        const url = templateObject.getRequestURLofJSONP();
        $.ajax({
            url: url,
            method: method,
            dataType: 'JSONP',
            ...
            success: function(responseJSON) {
                ...
                if (result instanceof Array) {
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        if (keys[i] === 'labellist' || keys[i] ===  'pets') {
                ...
                // 插入模板
                if($container === templateObject.$container && (dataPackage.content.length > 0 || Object.keys(dataPackage).length > 1)) {
