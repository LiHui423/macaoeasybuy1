...
function shopKey(){
	$.ajax({
		url:"http://social1.macaoeasybuy.com/SolrShopsController/QuerySolrRandomShops.easy?easybuyCallback=?",
		type:"GET",
		datatype:"jsonp",
		success:function(data){
...
function labelKey(){
	$.ajax({
		url:"http://social1.macaoeasybuy.com/SolrLabelsController/QueryHotSearchLabels.easy?easybuyCallback=?",
		type:"GET",
		datatype:"jsonp",
		success:function(data){
...
function topicKey(){
	$.ajax({
		url:"http://social1.macaoeasybuy.com/SolrTopicsController/queryHotSearchTopic.easy?easybuyCallback=?",
		type:"GET",
		datatype:"jsonp",
		success:function(data){
...
function postKey(){
	$.ajax({
		url:"http://social1.macaoeasybuy.com/SolrPostsController/QuerySearchPostsInfo.easy?easybuyCallback=?",
		type:"GET",
		datatype:"jsonp",
		success:function(data){
...
function fansKey(){
	$.ajax({
		url:"http://social1.macaoeasybuy.com/SolrUsersController/QueryCommandSearchUser.easy?easybuyCallback=?",
		type:"GET",
		datatype:"jsonp",
		success:function(data){
...
function shopLoad(keyword){
	$.getJSON("http://social1.macaoeasybuy.com/SolrShopsController/QueryShopsForTitle.easy?Query="+ keyword +"&easybuyCallback=?",function(data){
		var html = '';
		if(data.list.length == 0||data.list == null){
			html = '<li class="typeNormal"><p>暫無推薦選項</p></li>';
		}else{
			for(var i = 0;i < data.list.length;i++){
				if(data.list[i].shopLogo == ''){
					data.list[i].shopLogo = '/src/src/img/common/loading_pc_shop.png'
				}else{
