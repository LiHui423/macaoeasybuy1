...
function pickerDateRange(inputId, options) {
    ...
    var defaults = {
        ...
        defaultText: ' 至 ',
        singleCompare: false,
        stopToday: false, //是否取消之後的日期
        startToday: true, //是否取消之前的日期
        isTodayValid: true, //當天是否可選
        ...
        isClickCancel:false,//選好之後是否關閉日曆
        isScrollCancel : true //滾動是否取消彈框
    };
    //将对象赋给__method变量
    var __method = this;
    this.isSelectHide = options.isSelectHide;
    ...
    if(this.mOpts.isScrollCancel){
	    $(document).on('scroll.range', function () {
	    	$("#" + __method.calendarId).css('display', 'none');
	        __method.close(1);
	    });
    }
...
pickerDateRange.prototype.addCSS = function (isCompare, specialClass) {
    ...
    if (this.mOpts.theme == 'ta') {
        ...
        if($('#'+this.startDateId).val().length >0 && $('#'+this.endDateId).val().length == 0){
			$('#' + this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-')).removeClass().addClass('today');
		}
        //如果开始结束时间相同
...
pickerDateRange.prototype.checkDateRange = function (startYmd, endYmd) {
    ...
    if (eTime >= sTime) {
        ...
        if (maxEDate.getTime() < eTime) {
            alert('時間範圍一個月，結束日期不能大於：' + this.date2ymd(maxEDate).join('-'));
        ...
        maxEDate.setDate(maxEDate.getDate() - this.mOpts.dayRangeMax + 1);
        if (maxEDate.getTime() > eTime) {
            alert('時間範圍一個月，開始日期不能小於：' + this.date2ymd(maxEDate).join('-'));
            $("#" + this.calendarId).css('display', 'none');
            this.close(1);
            return false;
        }
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
            ...
            if(this.mOpts.isClickCancel)  $("#" + this.calendarId).css('display', 'none');
        } else {
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    $("#" + this.calendarId).css('top', pos.top + $('#'+this.inputId).outerHeight(true) + 'px');
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    if (btnSubmit) {
    	var nowStart = $('#' + __method.startDateId).val();
	    var nowEnd = $('#' + __method.endDateId).val();
	    if(nowStart.length>0 && nowEnd.length==0){
	    	$('#' + __method.startDateId).val(__method.mOpts.oldoldStart);
	    	$('#' + __method.endDateId).val(__method.mOpts.oldoldEnd);
	    	nowStart = null;
	    	nowEnd = null;
	    }
        //如果是单日快捷选择
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    if (isTaTheme) {
        ...
        if (this.mOpts.magicSelect) {
            ...
            $(cap).append(year + '<span>年</span>' + (month + 1) + '<span>月</span>');
        }
        ...
        if (0 == deviation) {
            ...
            (function (ymd) {
                $(td).bind("click", ymd, function () {
                	__method.mOpts.oldoldStart = $('#'+__method.mOpts.startDateId).val();
                	__method.mOpts.oldoldEnd = $('#'+__method.mOpts.endDateId).val();
