...
var requestObj = {
	...
	balckNum : {
		page : 0, isComplete : false, isFirst : false, showBoxId : 'balckNum'
	},
	loveNums : {
...
function getListdata(){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.userReleaseList.isSelf == 1){
...
function removeSelect(data){
	var dataList = data.userReleaseList.releaseList;
	...
	$('#delete-post .sure-cancel').on('click',function(){
		...
		$.getJSON('http://userspace.macaoeasybuy.com/UserDiaryConntroller/deleteBatchRelease.easy?idStr='+idStr+'&userId='+userId+'&integral='+arr.length * deleteIntegral+'&easybuyCallback=?',function(data){
			if(data.status == 'success'){
				...
				requestObj.balckNum.isFirst = true;
