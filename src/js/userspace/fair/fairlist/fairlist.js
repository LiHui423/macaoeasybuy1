...
function deleteIntegraFunc(){
	$.getJSON('http://userspace1.macaoeasybuy.com/integralController/queryTopicIntegral.easy?type=releaseFair&easybuyCallback=?',function(data){
...
function getListdata(){
	...
	var order = type;
	var dataUrl = 'http://userspace1.macaoeasybuy.com/UserFairsConntroller/queryUserFairs.easy';
...
function removeSelect(data){
	var dataList = data.fairList.fairList;
...
function deletePostTips(){
	...
	$('#delete-post .sure-cancel').on('click',function(){
		...
		$.getJSON('http://userspace1.macaoeasybuy.com/UserFairsConntroller/deleteBatchUserFairs.easy?idStr='+idStr+'&userId='+userId+'&integral='+arr.length * deleteIntegral+'&easybuyCallback=?',function(data){
